...
class rectustuho(discord.ui.Modal):
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        await interaction.followup.send(f"{interaction.user.mention}が<@&{festint.Bravo_role}>募集中！: {spla3.is_persistent(spla3())}", embed=embed, file=f, view=spla3())

class ustuhocmd(commands.Cog):
    def __init__(self, bot):
        self.bot = bot

    @slash_command(name=f"ぼしゅう{festint.Bravo_name}陣営" ,guild_ids=guild_ids, description=f"{festint.Bravo_name}陣営のフェス募集を取り付けます。")
    async def ustuhorect(self, interaction: discord.Interaction):  
        if interaction.channel.id not in [{festint.Bravo_channel},802345513495822339,803028814736392192]:
            return await interaction.respond(f"エラー：{festint.Bravo_name}陣営募集コマンドは <#{festint.Bravo_channel}> で実行して下さい。",ephemeral = True)   
    ...
    async def on_message(self,message):
        if message.author.bot:
            if message.channel.id not in [{festint.Bravo_channel},802345513495822339,803028814736392192]:
                return
            rectword=[f'が<@&{festint.Bravo_role}>募集中！']
            for word in rectword:
                if word in message.content:
                    await message.channel.send(f"@everyone<@&{festint.Bravo_role}>", delete_after=5)
            return 
