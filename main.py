...
async def on_ready():
    ...
    db2 = sqlite3.connect('geso.sqlite')
    cursor = db2.cursor()
    cursor.execute(
            'CREATE TABLE IF NOT EXISTS main.geso(guild_id TEXT, user_id TEXT,brand TEXT,gear TEXT,power TEXT)')
    cursor.execute(
             'CREATE TABLE IF NOT EXISTS geso(guild_id TEXT, user_id TEXT,brand TEXT,gear TEXT,power TEXT)')
    
    db3 = sqlite3.connect('geso2.sqlite')
    cursor = db3.cursor()
    cursor.execute(
            'CREATE TABLE IF NOT EXISTS main.geso(guild_id TEXT, user_id TEXT,brand TEXT,gear TEXT,power TEXT)')
    cursor.execute(
    ...
    db4 = sqlite3.connect('geso3.sqlite')
    cursor = db4.cursor()
    cursor.execute(
            'CREATE TABLE IF NOT EXISTS main.geso(guild_id TEXT, user_id TEXT,brand TEXT,gear TEXT,power TEXT)')
    cursor.execute(
...
bot.load_extension("cogs.gesotown.select")
